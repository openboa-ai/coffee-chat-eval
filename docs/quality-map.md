...
| Acceptance criteria   | Trial tuples bind evaluator and host configuration; receipts retain redacted inspectable evidence and cleanup; status remains explicit; dry run has no score.                                              |
| Failure modes         | Duplicate or under-specified identity, unproven isolation, secret leakage, malformed artifact misattribution, non-JSON metric, missing artifact treated as zero, collapsed failure owner, missing cleanup. |
