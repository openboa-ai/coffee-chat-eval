...
function familiesFor(profile: TasteProfile): readonly TasteFamily[] {
  const count =
    profile === "fixture" || profile === "smoke" || profile === "pilot"
      ? 1
      : TASTE_FAMILY_COUNT;
...
export interface TasteBenchApi {
  readonly getBenchmarkInput: (manifest: unknown, condition: TasteCondition) => unknown;
...
function parseCandidateSubmission(artifact: PrivateArtifactRef): CandidateSubmission {
  ...
  try {
    ...
    throw new TypeError(
      "Taste candidate output must be structured CandidateSubmission JSON",
    );
  }
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TypeError("Taste candidate submission must be an object");
  const record = value as Record<string, unknown>;
  if (
    JSON.stringify(Object.keys(record).sort()) !==
    JSON.stringify(["artifact", "decisionRecord"])
  )
    throw new TypeError("Taste candidate submission has unexpected fields");
  ...
  if (
    ...
    throw new TypeError("Taste candidate artifact is invalid");
  const artifactRecord = rawArtifact as Record<string, unknown>;
  if (
    artifactRecord.mediaType !== "text/plain" ||
    typeof artifactRecord.content !== "string" ||
    artifactRecord.content.trim().length === 0
  )
    throw new TypeError("Taste candidate artifact is invalid");
  if (
    rawDecision === null ||
    ...
    throw new TypeError("Taste decision record is invalid");
  const decisionRecord = rawDecision as Record<string, unknown>;
  if (
    JSON.stringify(Object.keys(decisionRecord).sort()) !==
    JSON.stringify([
      "constraints",
      "decision",
      "evidenceUse",
    ...
    throw new TypeError("Taste decision record has unexpected fields");
  if (
    typeof decisionRecord.decision !== "string" ||
    decisionRecord.decision.trim().length === 0
  )
    throw new TypeError("Taste decision is invalid");
  if (
    decisionRecord.uncertainty !== null &&
    typeof decisionRecord.uncertainty !== "string"
  )
    throw new TypeError("Taste uncertainty is invalid");
  ...
  if (
    !Array.isArray(evidenceUse) ||
    !Array.isArray(tradeoffs) ||
    !Array.isArray(constraints)
  )
    throw new TypeError("Taste decision arrays are invalid");
  return Object.freeze({
    ...
    decisionRecord: Object.freeze({
      ...
      evidenceUse: Object.freeze(
        evidenceUse.map((entry) => {
          if (entry === null || typeof entry !== "object" || Array.isArray(entry))
            throw new TypeError("Taste evidenceUse entry is invalid");
          const value = entry as Record<string, unknown>;
          if (typeof value.sourceId !== "string" || typeof value.use !== "string")
            throw new TypeError("Taste evidenceUse entry is invalid");
      ...
      tradeoffs: Object.freeze(
        tradeoffs.map((entry) => {
          if (entry === null || typeof entry !== "object" || Array.isArray(entry))
            throw new TypeError("Taste tradeoff is invalid");
          const value = entry as Record<string, unknown>;
          if (
          ...
          return Object.freeze({
            factors: Object.freeze([
              value.factors[0] as string,
              value.factors[1] as string,
      ...
      constraints: Object.freeze(
        constraints.map((entry) => {
          if (entry === null || typeof entry !== "object" || Array.isArray(entry))
            throw new TypeError("Taste constraint is invalid");
          const value = entry as Record<string, unknown>;
          if (
...
export async function executeTasteBench(input: {
  ...
  readonly manifest: unknown;
  readonly manifestForFamily?: ((family: TasteFamily) => unknown) | undefined;
  ...
  try {
    for (const family of families) {
      const familyManifest = input.manifestForFamily?.(family) ?? input.manifest;
      const submissions: Partial<Record<TasteCondition, CandidateSubmission>> = {};
      for (const condition of TASTE_CONDITIONS) {
        phase = "source";
        const benchmarkInput = input.api.getBenchmarkInput(familyManifest, condition);
        phase = "candidate";
        const output = await input.candidate.run({
          familyId: family.familyId,
          condition,
          benchmarkInput,
        });
      ...
      nativeEvaluation = await input.api.evaluateCaseFamily({
...
export function createTasteTrackExecutor(
  options: { readonly api?: TasteBenchApi } = {},
) {
  return async (context: {
    readonly plan: {
      ...
      readonly trackId: "coffee-chat-taste";
    };
    readonly manifest: unknown;
    ...
    if (context.judge === undefined) {
      return Object.freeze({
        ...
        nativeEvidence: context.evidence({
          value: { reason: "Taste Judge capability is missing" },
    ...
    let api = options.api;
    let manifestForFamily: ((family: TasteFamily) => unknown) | undefined;
    ...
    if (api === undefined) {
      api = (await import(
        resolve(context.source.sourceRoot, "src/evaluator.ts")
      )) as TasteBenchApi;
      const bank = JSON.parse(
        readFileSync(resolve(context.source.sourceRoot, "bank/bank.json"), "utf8"),
      ) as { readonly cases?: readonly { readonly casePath: string }[] };
      if (!Array.isArray(bank.cases) || bank.cases.length === 0) {
        throw new TypeError("Bench bank has no cases");
      }
      manifestForFamily = (family) => {
        const entry = bank.cases?.[family.ordinal];
        if (entry === undefined)
          throw new TypeError(`Bench family is missing: ${family.familyId}`);
        const path = resolve(context.source.sourceRoot, "bank", entry.casePath);
        const root = resolve(context.source.sourceRoot);
        if (path !== root && !path.startsWith(`${root}/`))
          throw new TypeError("Bench case escapes source root");
    ...
    const metrics = Object.freeze({
      pointwiseCalls: Object.freeze({
        numerator: summary.judgeCalls >= 13 ? 13 : summary.judgeCalls,
        denominator: 13,
        value: null,
      }),
      pairwiseCalls: Object.freeze({
        numerator: summary.judgeCalls >= 21 ? 8 : Math.max(0, summary.judgeCalls - 13),
        denominator: 8,
        value: null,
      }),
    });
    ...
    for (const [index, artifact] of (summary.candidateArtifacts ?? []).entries()) {
      trialReceipts.push(
        createTrialReceipt({
          ...
          executionStatus:
            summary.executionStatus === "measured"
              ? "measured"
    ...
    return Object.freeze({
      ...
      ...(summary.failureOwner === undefined
        ? {}
        : { failureOwner: summary.failureOwner }),
      trialReceipts: Object.freeze(trialReceipts),
      metrics,
      nativeEvidence: context.evidence({
        value: { schema: "coffee-chat-eval/taste-native-v1", summary },
        mediaType: "application/json",
      }),
      cleanupStatus: "complete" as const,
    });
